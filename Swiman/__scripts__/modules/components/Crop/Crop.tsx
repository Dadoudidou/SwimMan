...
function isCropInvalid(crop: ICropObj) {
    return !crop.width || !crop.height;
}

function inverseOrd(ord: 'n' | 'ne' | 'e' | 'se' | 's' | 'sw' | 'w' | 'nw') {
    let inversedOrd;
...
function ensureAspectDimensions(cropObj: ICropObj, imageEl) {
    ...
    if (crop.width) {
        crop.height = (crop.width / (crop.aspect as number)) * imageAspect;
    } else if (crop.height) {
        crop.width = (crop.height * (crop.aspect as number)) / imageAspect;
    }
...
interface ICropProps {
    ...
    onChange: (crop: ICropObj, pixelCrop: IPixelCrop) => void
    onComplete: (crop: ICropObj, pixelCrop: IPixelCrop) => void
    onImageLoaded: (crop: ICropObj, imageEl: HTMLImageElement, pixelCrop: IPixelCrop) => void
    onAspectRatioChange: (nextCrop: ICropObj, pixelNextCrop: IPixelCrop) => void
    disabled: boolean
    crossorigin: string
}
...
class Crop extends React.PureComponent<ICropProps, ICropState>{
    ...
    //#region PRIVATE
    cropInvalid: boolean = false
    imageRef: HTMLImageElement = undefined;
    imageCopyRef: HTMLImageElement = undefined;
    mouseDownOnCrop: boolean = undefined;
    evData = undefined
    componentRef = undefined
    cropSelectRef = undefined;
    cropWrapperRef = undefined


    nextCropState(crop: ICropObj): ICropObj {
        const nextCrop = { ...Crop.defaultCrop, ...crop };
        this.cropInvalid = isCropInvalid(nextCrop);
        return nextCrop;
    }

    getPixelCrop(crop: ICropObj): IPixelCrop {
        return {
    ...
    getNewSize() {
        ...
        if (crop.aspect) {
            newHeight = (newWidth / (crop.aspect as number)) * imageAspect;
        } else {
        ...
        if (crop.aspect) {
            newWidth = clamp((newHeight * (crop.aspect as number)) / imageAspect, 0, 100);
        }
    ...
    componentDidMount() {
        ...
        if ((this.imageRef as any).complete || (this.imageRef as any).readyState) {
    ...
    componentWillReceiveProps(nextProps: ICropProps) {
        if (nextProps.crop) {
    ...
    onDocMouseTouchMove(e: TouchEvent | MouseEvent) {
    ...
    onDocMouseTouchEnd(e: TouchEvent | MouseEvent) {
        if (this.props.disabled) {
            return;
        }
    ...
    onImageLoad(imageEl: HTMLImageElement) {
    ...
    onComponentMouseTouchDown(e: any) {
        ...
        this.setState({ ...this.state, newCropIsBeingDrawn: true });
    }
    ...
    onCropMouseTouchDown(e: any) {
        ...
        const ord = (e.target as any).dataset.ord;
